...
using ImageClassificationSample;
    ...
    internal sealed partial class ImageClassificationResNet : Page
    {
        ...
        private Task InitModel(string modelPath)
        {
            return Task.Run(() =>
            {
                ...
                try
                {
                    var (dmlDevice, commandQueue) = DMLHelpers.CreateDmlDeviceAndCommandQueue("NPU");

                    sessionOptions.EnableMemoryPattern = false;
                    sessionOptions.ExecutionMode = ExecutionMode.ORT_SEQUENTIAL;
                    sessionOptions.AppendExecutionProvider_DML1(Marshal.GetIUnknownForObject(dmlDevice), Marshal.GetIUnknownForObject(commandQueue));
